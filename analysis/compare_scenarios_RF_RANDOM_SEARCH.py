...
FILE_RS = os.path.join(DATA_DIR, "results_Random_Search_Random_Forest_on_NOISE.csv")
...
def main():
    print("Analisando Random Search (Random Forest)")
    ...
    data = {
        "Cenário": ["Random Search (Random Forest + Noise)"],
    ...
    df_table.to_csv("resumo_random_search_random_forest.csv", index=False)
    ...
    plt.title('Convergência: Random Search (Random Forest)', fontsize=14)
    plt.xlabel('Número de Avaliações (Trials)', fontsize=16)
    plt.ylabel('Melhor F1-Score Acumulado', fontsize=16)
    ...
    # Salva em PDF e PNG
    plt.savefig("analise_random_search_random_forest.pdf", format='pdf', dpi=300, bbox_inches='tight')
    plt.savefig("analise_random_search_random_forest.png", dpi=300, bbox_inches='tight')
