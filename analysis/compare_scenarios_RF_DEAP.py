...
# CONFIGURAÇÃO DOS ARQUIVOS (RANDOM FOREST)
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.abspath(os.path.join(CURRENT_DIR, "..", ".."))
DATA_DIR = os.path.join(PROJECT_ROOT, "src", "optimization")

# NOMES DOS ARQUIVOS
FILE_ORIGINAL = os.path.join(DATA_DIR, "results_RANDOM_FOREST_on_original.csv")
FILE_SHIFT = os.path.join(DATA_DIR, "results_RANDOM_FOREST_on_SHIFT.csv")
FILE_NOISE = os.path.join(DATA_DIR, "results_RANDOM_FOREST_on_NOISE.csv")
...
def main():
    print("Gerando Comparativo RANDOM FOREST")

    # Carregar dados
    df_orig, f1_orig = get_data(FILE_ORIGINAL, "RF Original")
    df_shift, f1_shift = get_data(FILE_SHIFT, "RF Shift")
    df_noise, f1_noise = get_data(FILE_NOISE, "RF Noise")
    ...
    df_table.to_csv("tabela_comparativa_RF.csv", index=False)
    ...
    plt.title('Random Forest: Comparação de Estratégias', fontsize=18)
    ...
    plt.tight_layout()
    plt.savefig("comparativo_RANDOM_FOREST_barras.pdf", format='pdf', dpi=300, bbox_inches='tight')
    plt.savefig("comparativo_RANDOM_FOREST_barras.png", dpi=300, bbox_inches='tight')
    ...
    plt.title('Evolução da Otimização (Random Forest)', fontsize=18)
    ...
    plt.tight_layout()
    plt.savefig("comparativo_RANDOM_FOREST_curvas.pdf", format='pdf', dpi=300, bbox_inches='tight')
    plt.savefig("comparativo_RANDOM_FOREST_curvas.png", dpi=300, bbox_inches='tight')
